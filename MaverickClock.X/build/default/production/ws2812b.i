...
static __attribute__((inline)) void WS2812B_Reset( );
                                 ...
                                 _Bool * const wasSetupSuccessful )
{
    ...
    *wasSetupSuccessful = ( ( ((void*)0) == pxBuff ) || ( 0 == numElements ) ) ? 0 : 1;
    return array;
}
# 56 "ws2812b.c"
void WS2812B_Render( const ws2812bArray * const strip )
{
    ...
    for( i = 0; i < strip->numPixels; i++ )
    {
        ...
        for( j = 0; j < 8u; j++ )
        {
            ...
            {
                do { LATCbits.LATC2 = 1; } while(0); __nop(); __nop(); __nop(); __nop(); do { LATCbits.LATC2 = 0; } while(0); __nop(); __nop();;
            }
            else
            {
                do { LATCbits.LATC2 = 1; } while(0); __nop(); do { LATCbits.LATC2 = 0; } while(0); __nop(); __nop(); __nop(); __nop(); __nop();;
            }
        }
        for( j = 0; j < 8u; j++ )
        {
                                         ...
                                         const uint8_t blue )
{
    ...
    size_t i;
    uint8_t rgbArray[3u] = {red, green, blue};
    uint8_t * startPixelAddress = &( strip->pixelBuffer[pixelStartOffset].red );
    for( i = 0; i < numPixelsToSet; i++ )
    {
        memcpy( ( startPixelAddress + 3u * i ), rgbArray, 3u );
    }
    return;
}
                                             ...
                                             const size_t numPixelsToCopy )
{
    ...
    memcpy( dest, src, numPixelsToCopy * 3u );
    return;
}
