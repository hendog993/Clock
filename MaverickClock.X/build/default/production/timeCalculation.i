...
# 22 "timeCalculation.c" 2



static volatile uint32_t currentTimeInSeconds = 0u;
# 43 "timeCalculation.c"
...
void Time_RenderIfMinutesHaveChanged( _Bool isSwitchPressHrMnMode )
{
    ...
    static uint16_t minutesAtLastChange = 0xFFFF;
    ...
    int32_t thisTimeInSeconds = (int32_t) currentTimeInSeconds + ( minutesDelta * 60 );
    thisTimeInSeconds = ( (thisTimeInSeconds < 0) ? 43199u : (thisTimeInSeconds > 43199u) ? 0 : thisTimeInSeconds);
    currentTimeInSeconds = ( volatile uint32_t ) thisTimeInSeconds;
    uint16_t currentTimeInMinutes = (uint16_t) ( currentTimeInSeconds / 60u );


    if( currentTimeInMinutes != minutesAtLastChange )
    {
        minutesAtLastChange = currentTimeInMinutes;
        uint8_t hours = (uint8_t) ( currentTimeInMinutes / 60u ) + 1u;
        uint8_t minutes = (uint8_t) ( currentTimeInMinutes % 60u );
...
void Time_RenderInputTime( uint32_t timeInSeconds,
                           _Bool setCurrentTime )
{
    uint16_t currentTimeInMinutes = (uint16_t) ( timeInSeconds / 60u );
...
static void Time_IncrementSecondsISR( void )
{
    currentTimeInSeconds = ( currentTimeInSeconds + 1 > 43199u ) ? 0 : currentTimeInSeconds + 1;
    return;
}
